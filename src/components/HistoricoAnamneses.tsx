...
export function HistoricoAnamneses() {
  const [registros, setRegistros] = useState<StoredRecord[]>([])
  ...
  useEffect(() => {
    ...
    const carregar = () => {
      listRecords()
        ...
        .catch((err) => {
          console.error('Erro ao listar registros do IndexedDB:', err)
          if (ativo) {
            setRegistros([])
            setCarregando(false)
          }
    ...
    window.addEventListener('anamnese-storage-updated', carregar)
    document.addEventListener('visibilitychange', handleVisibilityChange)
    return () => {
      ativo = false
      window.removeEventListener('focus', carregar)
      window.removeEventListener('anamnese-storage-updated', carregar)
  ...
  if (carregando || registros.length === 0) return null
  ...
  const confirmarExclusao = async () => {
    ...
    try {
      ...
      setRegistros((atuais) => atuais.filter((registro) => registro.id !== id))
    } finally {
      setExcluindo(null)
    }
  }
